...
def extract_fields(row: Dict[str, Any]) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """
    Lingo/general datasets might still have images, or might be text-only.
    If image is missing, we still output image_path="" (empty string).
    """
    img = None
    ...
    q = None
    for k in ("question", "Question", "query", "Q", "prompt", "text"):
        if k in row and row[k]:
            q = str(row[k])
            break

    a = None
    for k in ("answer", "Answer", "A", "ans", "label", "target"):
...
def build_records(
    ...
    for i in range(n_take):
        ...
        records.append(
            {
                "id": f"lingoqa_{split}_{i+1:06d}",
                "image_path": img_path,  # may be empty if dataset is text-only
                "question": q.strip(),
                "answer": a.strip(),
                "domain": domain,
            }
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Preprocess LingoQA-like dataset into unified JSONL format.")
    p.add_argument("--raw_dir", type=str, required=True)
    p.add_argument("--out_dir", type=str, required=True)
    p.add_argument("--copy_images", type=str, default="false")
    p.add_argument("--limit", type=int, default=0)
    p.add_argument("--domain", type=str, default="linguistic")
    # explicit overrides
...
def main() -> int:
    ...
    if not split_files:
        raise SystemExit(
            f"[LingoQA] Could not auto-detect annotation files in {raw_dir}.\n"
            f"Provide --train_ann/--val_ann/--test_ann explicitly."
        )

    print("[LingoQA] raw_dir:", raw_dir)
    print("[LingoQA] images_dir:", images_dir)
    print("[LingoQA] out_dir:", out_dir)
    print("[LingoQA] split_files:", {k: str(v) for k, v in split_files.items()})
    ...
    for split, ann_path in split_files.items():
        ...
        print(f"[LingoQA] wrote {n} records -> {out_path}")
        for s in recs[:2]:
            print("  sample:", s)
